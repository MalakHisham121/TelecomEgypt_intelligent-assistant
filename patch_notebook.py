...
with open("TelecomEgypt_Assistant.ipynb", "r") as f:
    nb = json.load(f)

new_source = """import gradio as gr
...
def process_interaction(audio_filepath, file_paths, text_input, history):
    ...
    if file_paths:
        ...
        for fp in file_paths:
            ...
            if docs:
                for d in docs:
                    uploaded_context += f"\\n[Uploaded Document Content]: {d.page_content}\\n"
    ...
    if not user_input and uploaded_context:
        user_input = "Please summarize the content of the uploaded document(s)."
    ...
    # Append the uploaded text directly to user input so LLM knows about it explicitly
    if uploaded_context:
        user_input = f"{user_input}\\n\\nHere is the content of the uploaded document(s) for your immediate reference:\\n{uploaded_context}"
    ...
    # Add back the display version of user input to history (without the huge document text)
...
nb["cells"][26]["source"] = new_source_lines

with open("TelecomEgypt_Assistant.ipynb", "w") as f:
